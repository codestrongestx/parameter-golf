...
class Hyperparameters:
    ...
    # Model (defaults follow the DepthFiLM recurrent demo from the plan).
    vocab_size: int = int(os.environ.get("VOCAB_SIZE", 1024))
    num_layers: int = int(os.environ.get("NUM_LAYERS", 12))
    num_unique_layers: int = int(os.environ.get("NUM_UNIQUE_LAYERS", 4))
    ...
    depth_condition_dim: int = int(os.environ.get("DEPTH_CONDITION_DIM", 32))
    depth_condition_scale: float = float(os.environ.get("DEPTH_CONDITION_SCALE", 0.10))
    depth_condition_init_std: float = float(os.environ.get("DEPTH_CONDITION_INIT_STD", 0.02))
...
class DepthFiLM(nn.Module):
    def __init__(self, depth_dim: int, dim: int, scale: float):
        ...
        self.attn_proj.weight = mx.zeros_like(self.attn_proj.weight).astype(mx.float32)
        self.attn_proj.bias = mx.zeros_like(self.attn_proj.bias).astype(mx.float32)
        self.mlp_proj.weight = mx.zeros_like(self.mlp_proj.weight).astype(mx.float32)
        self.mlp_proj.bias = mx.zeros_like(self.mlp_proj.bias).astype(mx.float32)

    def __call__(self, depth_code: mx.array) -> tuple[mx.array, mx.array, mx.array, mx.array]:
        depth = depth_code[None, :].astype(mx.float32)
        attn = self.attn_proj(depth)[0]
        mlp = self.mlp_proj(depth)[0]
        attn_gamma = 1.0 + self.scale * mx.tanh(attn[: self.dim])
        attn_beta = self.scale * mx.tanh(attn[self.dim :])
        mlp_gamma = 1.0 + self.scale * mx.tanh(mlp[: self.dim])
        mlp_beta = self.scale * mx.tanh(mlp[self.dim :])
...
class Block(nn.Module):
    ...
    def __call__(self, x: mx.array, x0: mx.array, depth_code: mx.array | None) -> mx.array:
        ...
        if self.depth_film is not None and depth_code is not None:
            ...
            attn_in = attn_in * attn_gamma.astype(x.dtype)[None, None, :] + attn_beta.astype(x.dtype)[None, None, :]
        attn_out = self.attn(attn_in)
        x = x + self.attn_scale.astype(x.dtype)[None, None, :] * attn_out
        mlp_in = self.mlp_norm(x)
        if mlp_gamma is not None and mlp_beta is not None:
            mlp_in = mlp_in * mlp_gamma.astype(x.dtype)[None, None, :] + mlp_beta.astype(x.dtype)[None, None, :]
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.block_schedule = tuple(i % self.num_unique_layers for i in range(num_layers))
        ...
        if depth_condition_dim > 0:
            self.depth_codes = (
                mx.random.normal((num_layers, depth_condition_dim), dtype=mx.float32) * depth_condition_init_std
            ).astype(mx.float32)
        self.final_norm = RMSNormNoWeight()

        for b in self.unique_blocks:
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        for i in range(self.num_encoder_layers):
            block = self.unique_blocks[self.block_schedule[i]]
            depth_code = None if self.depth_codes is None else self.depth_codes[i]
            x = block(x, x0, depth_code)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            block = self.unique_blocks[self.block_schedule[block_idx]]
            depth_code = None if self.depth_codes is None else self.depth_codes[block_idx]
            x = block(x, x0, depth_code)
...
def main() -> None:
    ...
    log(
        f"model_params:{n_params} vocab_size:{args.vocab_size} virtual_layers:{args.num_layers} "
        f"unique_layers:{args.num_unique_layers} depth_condition_dim:{args.depth_condition_dim} "
        f"dim:{args.model_dim} heads:{args.num_heads} kv_heads:{args.num_kv_heads} "
        f"mlp_mult:{args.mlp_mult} seq_len:{args.train_seq_len} tie_embeddings:{args.tie_embeddings}"
    )
    log(
    ...
    log(
        ...
        f"linear_weight:{model.unique_blocks[0].attn.c_q.weight.dtype} "
    ...
    # model.state also includes static Python metadata such as block_schedule.
    # Persist only actual MLX tensors; the structural metadata is rebuilt by code.
